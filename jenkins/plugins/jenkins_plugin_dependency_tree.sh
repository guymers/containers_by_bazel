...
function resolveDependencies() {
  ...
  local dependency_str="$(unzip -p "$jpi" META-INF/MANIFEST.MF | tr -d '\r' | tr '\n' '|' | sed -e 's#| ##g' | tr '|' '\n' | grep "^Plugin-Dependencies: " | sed -e 's#^Plugin-Dependencies: ##')"
  ...
  for dependency in "${dependencies[@]}"; do
    local plugin="$(echo "$dependency" | cut -d ';' -f1)"
...
function downloadAndResolveDeps() {
  local v=$(download "$1" "$2")
  local depRName=$(echo "$v" | cut -d ' ' -f1)
  local depRVersion=$(echo "$v" | cut -d ' ' -f2)
